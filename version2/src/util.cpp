...
void Timer::Entry::start() {
    erriff(_start != 0.0, "entry not started");
...
void Timer::Entry::end(const std::string &desc) {
    erriff(_start == 0.0, "entry not started");
    erriff(_desc != desc, "timer end out of sequence");
...
double Timer::getTime_msec() {
    ...
    erriff(0 != clock_gettime(CLOCK_REALTIME, &curr), "clock_gettime");
...
void load_file(const char *path, void **ret_buf, size_t *ret_n) {
    ...
    int rc = stat(path, &s);
    erriff(rc == -1, "failed stating file %s", path);

    size_t n = s.st_size;
    char *buf = (char *)malloc(n + 1);
    erriff(!buf, "failed allocating %lu bytes for sequence", *ret_n);

    FILE *f = fopen(path, "r");
    erriff(!f, "failed opening %s", path);

    dbf("reading %lu bytes from %s...", n, path);

    size_t nread = fread(buf, n, 1, f);
    erriff(nread != 1, "failed reading entire file (%s)", path);
    buf[n] = 0;
